...
const cardDataURL =  // custom endpoint via ACF 
...
export async function getSortedPostsData() { // Read posts and return metadata sorted by date
  ...
  jsonObj.sort(function (a, b) { // Sort array by post title in alphabetical order
    return a.title.rendered.localeCompare(b.title.rendered); // Compare titles using locale-aware string comparison
  }); // End sort function
  ...
  return jsonObj.map(item => { // Transform each post object to include only needed fields
    return { // Return simplified post object
      ...
      title: item.acf?.title || '', // Use ACF card title field
      date: item.date || '', // Use post date
    } // End return object
  }) // End map function
...
export async function getPostData(id) { // Read one post and return HTML plus metadata
  ...
  return { // Return post data object
    ...
    date: objReturned.date || '', // Use post date
    // ACF fields
